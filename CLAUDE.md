...
>
> **The suite is NOT expected to be all-green on a plain checkout.** ~2503 pass, ~239 fail (out of 2801 tests). Judge regressions with `tests/__baseline__/verify-no-regression.mjs`, not a raw run. Expected red domains:
> - 239 catalogued in `tests/__baseline__/known-fails.txt`.
> - `tests/translator/golden-url-header.test.js` (126 fails — static snapshots need syncing with current endpoints/models).
> - `tests/unit/cursor-agent-proto.test.js` (35 fails — cursor agent protocol mocks).
> - `tests/unit/openai-to-kiro.test.js` & `tests/translator/claude-kiro-direct.test.js` (28 fails — thinking budgets & format mapping).
> - `tests/unit/security-audit.test.js` (13 fails — source-pinning regex checks).
> - `tests/unit/oauth-cursor-auto-import.test.js` (8 fails — cursor DB path discovery mocks).
