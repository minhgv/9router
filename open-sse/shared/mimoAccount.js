...
async function getServiceCookie(providerSpecificData, proxyOptions) {
  ...
  const promise = (async () => {
    try {
      ...
      return await acquireFn(passJar, proxyOptions, apiBase, providerSpecificData?.region);
    } catch (e) {
...
export function invalidateMimoAccountCookieCache(providerSpecificData = null) {
  ...
  if (passToken && typeof passToken === "string") {
    const apiBase = resolveMimoServerBase(typeof providerSpecificData === "string" ? null : providerSpecificData);
    const key = crypto.createHash("sha256").update(`${apiBase}|${passToken.trim()}`).digest("hex");
    _cache.delete(key);
  } else {
    _cache.clear();
  }
